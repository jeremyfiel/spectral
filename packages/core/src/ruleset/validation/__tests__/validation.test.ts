  ...
  describe('aliases validation', () => {
    ...
    it.each([null, 5, [], {}])('recognizes %p as an invalid type of aliases', alias => {
      expect(
        ...
        new AggregateError([
          new RulesetValidationError(
            'the value of an alias must be a valid JSON Path expression, a reference to the existing Alias optionally paired with a JSON Path expression subset, or contain a valid set of targets',
            ['aliases', 'alias'],
          ),
        ]),
      );
    ...
    it.each(['', 'foo'])('given %s value used as an alias, throws', value => {
      expect(
        ...
        new AggregateError([
          new RulesetValidationError(
            'the value of an alias must be a valid JSON Path expression or a reference to the existing Alias optionally paired with a JSON Path expression subset',
            ['aliases', 'PathItem'],
          ),
        ]),
      );
      ...
      it('recognizes invalid given', () => {
        expect(
          ...
          new AggregateError([
            new RulesetValidationError(
              `the value of an alias must be a valid JSON Path expression or a reference to the existing Alias optionally paired with a JSON Path expression subset`,
              ['aliases', 'SchemaObject', 'targets', '1', 'given'],
            ),
          ]),
        );
      });
